...
for path in paths:
    ...
    if (args.compile_complete is True) and (num_nc_files >= 50) and (len(non_full) == 0):
        ...
        print(f"\t(compile_complete) Now compiling final dataset for {args.institution} {experiment} {init}")
        if output_file.exists() is False:
            output_file.parent.mkdir(parents=True, exist_ok=True)
            mfds = xr.open_mfdataset(nc_files, combine="nested", concat_dim="member_id").load()
            print(f"\t(compile_complete) Writing compiled dataset to {output_file}")
            mfds.to_netcdf(output_file)
        else:
            print(f"\t(compile_complete) {output_file} already exists; skipping!")
